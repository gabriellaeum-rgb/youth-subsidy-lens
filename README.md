...
로그인 없음, 서버 저장 없음, 결정론적(rule-based) 매칭. 자세한 기획/디자인 근거는 `../docs/PRD_v3_청년지원금렌즈.md`, `../docs/design_spec_youthfundlens_v3_brand_update.md`, `../docs/branding_design_research_youthfundlens.md` 참고.
...
**현재 상태:** `public/data/programs.json`은 PRD AC4.8에서 정의한 3-row 스키마 예시(demo) 데이터입니다. 화면에는 "지금은 예시 데이터 3건으로 동작 중이에요" 안내가 노출되어 실제 매칭이 아님을 명시적으로 알립니다(F0-AC0.1 — 플레이스홀더를 숨기지 않고 정직하게 드러내는 쪽을 선택). 실제 32개 프로그램 8-attribute 정규화 큐레이션(약 1.5–2시간, CEO/가비 담당)이 완료되면 이 파일을 교체하세요 — 32개 이상이 채워지면 데모 안내 배너는 자동으로 사라집니다(`src/lib/usePrograms.ts`의 `DEMO_DATASET_THRESHOLD` 기준).
...
- **데이터셋이 3-row 예시(demo) 데이터입니다.** 실제 서비스 배포 전 32개 프로그램 큐레이션 필수 (위 "데이터 갱신 방법" 참고). 화면에 데모 안내 배너가 노출되는 동안은 실 서비스로 간주하지 않습니다.
